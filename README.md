...
All `/admin/*` routes are protected in deployed environments by the dependency-free root `middleware.js` using Vercel Routing Middleware and HTTP Basic Authentication. The middleware uses Vercel's default Edge runtime and returns Vercel's `x-middleware-next` continuation response for authorized requests. Vercel CLI 55 unconditionally launches `npm` while packaging Routing Middleware, even when the middleware imports no packages, but its Bun build image does not expose npm or Node to that subprocess. The Vercel install command therefore uses Bun to install npm 10, then links both npm and Bun's Node-compatible executable into Vercel's existing Yarn global bin directory, which is already on the packager's PATH. Bun remains the authoritative project package manager; `.npmrc` enables legacy peer resolution only for the isolated npm packaging step. Add both variables in Vercel Project Settings → Environment Variables for Production and Preview, then redeploy:
