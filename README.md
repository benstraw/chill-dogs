...
All `/admin/*` routes are protected in deployed environments by the dependency-free root `middleware.js` using Vercel Routing Middleware and HTTP Basic Authentication. The middleware uses Vercel's Bun runtime through `vercel.json` (`bunVersion: 1.x`) plus `config.runtime: 'nodejs'`. Vercel's framework-independent middleware packager still invokes npm after the Bun-managed site build, so npm is present as a project-local development dependency and `.npmrc` enables legacy peer resolution for that packaging step. Bun remains the authoritative project package manager. Add both variables in Vercel Project Settings → Environment Variables for Production and Preview, then redeploy:
